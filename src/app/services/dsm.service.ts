...
export class DSMService {
  ...
  public getMailingList( realm: string ): Observable<any> {
    ...
    let url = this.baseUrl + DSMService.UI + "mailingList";
    let map: { name: string, value: any }[] = [];
    map.push( {name: "userId", value: this.role.userID()} );
    map.push( {name: "userMail", value: this.role.userMail()} );
    map.push( {name: "realm", value: realm} );
    return this.http.get( url, this.buildQueryHeader( map ) ).map( ( res: Response ) => {
      res.json();
  ...
  private buildSimpleQueryHeader( map: any[] ): RequestOptions {
    let params: URLSearchParams = new URLSearchParams();
    for (let i in map) {
      params.append( map[ i ].name, map[ i ].value );
    }
    return new RequestOptions( {headers: this.buildJsonAuthHeaderNoCookieCheck(), withCredentials: true, search: params} );
  }
  ...
  private buildJsonAuthHeaderNoCookieCheck(): Headers {
    let headers = new Headers( {"Content-Type": "application/json", "Accept": "application/json"} );
    headers.append( "Authorization", this.sessionService.getAuthBearerHeaderValue() );
    return headers;
  }
  ...
  public refreshAccess(): Observable<any> {
    let url = this.baseUrl + DSMService.UI + "refreshAccess";
    let map: { name: string, value: any }[] = [];
    map.push( {name: "userId", value: this.role.userID()} );
    map.push( {name: "userMail", value: this.role.userMail()} );
    return this.http.get( url, this.buildSimpleQueryHeader( map ) ).map( ( res: Response ) => {
      res.json();
    } ).catch( this.handleError );
  }
}
