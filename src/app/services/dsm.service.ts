...
//    return Observable.throw( error );
  ...
  logErrorInErrorReporting( error: any ) {
//    let url = this.baseUrl + DSMService.UI + "errorReporting";
//    if (message === undefined) {
//      message = "";
//    }
//
//    message = "Error occured: " + message + " In realm: " + realm;
//    let map: { name: string, value: any }[] = [];
//    map.push( {name: DSMService.REALM, value: realm} );
//    map.push( {name: "functionName", value: functionName} );
//    map.push( {name: "message", value: message} );
//    map.push( {name: "eventTime", value: new Date().getUTCDate()} );
//    return this.http.post( url, null, this.buildQueryHeader( map ) ).map( ( res: Response ) => res.json() ).catch( this.handleError.bind(this) );
